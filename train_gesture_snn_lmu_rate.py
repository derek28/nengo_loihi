...
class LMUCell(nengo.Network):
    def __init__(self, hd, md, theta, xd, xn, **kwargs):
        ...
        with self:
            ...
            # self.u = [nengo.Ensemble(1, 1, neuron_type=Linear(), gain=np.ones(1), bias=np.zeros(1), label='Ensemble_u%d'%i).neurons for i in range(xd)]
            self.u = [nengo.Ensemble(1, 1, neuron_type=nengo.Tanh(tau_ref=1), gain=np.ones(1), bias=np.zeros(1), label='U%d'%i).neurons for i in range(xd)]
            # self.m = [nengo.Ensemble(md, 1, neuron_type=Linear(), gain=np.ones(md), bias=np.zeros(md), label='Ensemble_m%d'%i).neurons for i in range(xd)]
            self.m = [nengo.Ensemble(md, 1, neuron_type=nengo.Tanh(tau_ref=1), gain=np.ones(md), bias=np.zeros(md), label='M%d'%i).neurons for i in range(xd)]
            # self.h = nengo_dl.TensorNode(tf.nn.tanh, shape_in=(hd,), pass_time=False)
            self.h = nengo.Ensemble(hd, 1, neuron_type=nengo.Tanh(tau_ref=1), gain=np.ones(hd), bias=np.zeros(hd), label='H').neurons
...
do_training = True
n_epoch = 80
model_name = 'model_wiaug#4_memory#50_parallel#5_ensemble_tanh_nostate'
