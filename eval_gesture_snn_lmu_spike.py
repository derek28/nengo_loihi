...
with nengo.Network() as net:
    ...
    pu = [nengo.Probe(lmu.u[i], synapse=tau0) for i in range(n_chan)]
    ph = nengo.Probe(lmu.h[i], synapse=tau0)
    pm = [nengo.Probe(lmu.m[i], synapse=tau0) for i in range(n_chan)]


do_training = False
i_epoch = 79
n_epoch = 80
model_name = 'model_dt1ms_wiaug#4_memory#50_parallel#5_trainrate_testspike_tanh_delay#20'
...
with nengo_dl.Simulator(net, dt=dt, minibatch_size=120, unroll_simulation=10) as sim:
    ...
    sim.load_params("./weights/%s/params_epoch#%03d"%(model_name,i_epoch))
    # test_acc = sim.evaluate(x_test_dict, y_test, verbose=0)["probe_accuracy"]
    # print(f"Final test accuracy: {test_acc * 100:.2f}%")

    test_output = sim.predict(x_test_dict)
    test_predictions = np.argmax(test_output[p][:,-1], axis=-1)
    print("Test accuracy: %.2f%%" % (100*np.mean(test_predictions == y_test[:,0,0])))

    for i in range(n_chan):
        spikes = test_output[pu[i]]
        rates = ((spikes).sum(axis=1)/(dt*spikes.shape[1])).max()
        print("U%d: mean firing rate: %0.1f" % (i, rates))

    for i in range(n_chan):
        spikes = test_output[pm[i]]
        rates = ((np.abs(spikes)).sum(axis=1)/(dt*spikes.shape[1])).max()
        print("M%d: mean firing rate: %0.1f" % (i, rates))

    spikes = test_output[ph]
    rates = ((np.abs(spikes)).sum(axis=1)/(dt*spikes.shape[1])).max()
    print("H: mean firing rate: %0.1f" % (rates))
